...
//        Getting protocol selected from the previous activity
        ...
        if (getIntent().getExtras().getString("msg") != null && getIntent().getExtras().getString("msg") != ""){
            showMessage(getIntent().getExtras().getString("msg"));
        }
    }

//    Proceeding to Regsiter activity when register button is clicked
    public void goRegister(View view) {
        Intent registerActivity = new Intent(getApplicationContext(), RegisterActivityFirst.class);
...
//                        Creating a URL string with the query string to verify the email and password provided using remote database server
                        String serverPath = "http://"+dbIp+"/JavaChat/db.php?task=login&email="+email+"&password="+password+"&type=user";
